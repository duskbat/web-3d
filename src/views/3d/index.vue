...
export default {
  ...
  methods: {
    init(data) {
      ...
      let axis = new THREE.AxesHelper(1500);
      scene.add(axis);

      let glucose = data.glucose;
      let pointsParam = [];
      let list = [];
      pointsParam.push(list);

      var idx;
      let last;
      var forcnt = 0;
      for (idx in glucose) {
        var date = new Date(glucose[idx].x);
      ...
      for (curveidx in pointsParam) {
        ...
        let lineMaterial = new THREE.LineBasicMaterial({color: color[curveidx]});
