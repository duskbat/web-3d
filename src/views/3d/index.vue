...
export default {
  ...
  methods: {
    init(data) {
      ...
      let arridx = 0;
      let arr = new Float32Array(3*glucose.length);
      ...
      for (let i in pointsParam) {
        ...
        const curve = new THREE.CatmullRomCurve3(
            ...
            "centripetal",
            0.5);
        let points = curve.getPoints(pointList.length-1);
        ...
        for(let j in pointList){
          arr[arridx++] = pointList[j].x;
          arr[arridx++] = pointList[j].y;
          arr[arridx++] = pointList[j].z;
        }
        pgeometry.setAttribute( 'position', new THREE.BufferAttribute( arr, 3 ) );
        let pointsss = new THREE.Points(pgeometry, pMaterial);
        group.add(pointsss);
      }
